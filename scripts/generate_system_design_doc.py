...
add_para(
    'FlowMind Agent 采用经典的前后端分离架构。前端是 Vue 3 单页应用（SPA），通过 HTTP REST API 和 SSE 协议与后端通信。'
...
add_para('• 表示层（Presentation）：Vue 3 前端，Element Plus 组件，ECharts 图表，SSE EventSource', bold=False)
...
add_figure(
    '图 2-1  系统总体架构图',
    '该图展示了 FlowMind Agent 系统的完整架构，包括用户层、前端层、网关层、业务服务层、'
    'AI 智能体层和基础设施层。图中箭头表示依赖方向：前端依赖网关，网关依赖业务服务，'
    '业务服务依赖基础设施和 AI 智能体层。',
    '''
┌─────────────────────────────────────────────────────────────────────┐
│                         用户 / 浏览器 (Chrome / Edge)                │
└────────────────────────────┬────────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────────┐
│              Nginx / Vite Dev Server (开发阶段)                      │
│                     Frontend SPA                                    │
│        Vue 3 + TypeScript + Element Plus + ECharts                  │
│        ┌──────────┬──────────┬──────────┬──────────┐               │
│        │AI 工作台 │内容管理  │学员管理  │数据分析  │               │
│        │(SSE流式) │(选题/文案)│(画像/进度)│(ECharts)│               │
│        └──────────┴──────────┴──────────┴──────────┘               │
└────────────────────────────┬────────────────────────────────────────┘
                             │ REST / SSE
                             ▼
...
add_para(
    'MySQL 数据库名为 FlowMind，字符集使用 utf8mb4。按照业务域划分，共有 17 张核心表。每张表均包含 id（BIGINT 主键自增）、'
...
add_para('（以下数据待统计，提交最终文档时替换为实际值。）', bold=True)

add_table(
    ['类别', '代码量', '统计方式'],
    [
        ['客户端代码量（前端）', '待统计', '使用 cloc frontend/src/ --exclude-dir=node_modules,dist'],
        ['服务端代码量（后端）', '待统计', '使用 cloc backend/ --exclude-dir=target,.m2repo'],
        ['桌面客户端代码量', '待统计', '使用 cloc desktop_fronted/ --exclude-dir=.venv,__pycache__'],
        ['文档与配置文件', '待统计', '使用 cloc docs/ scripts/ --exclude-ext=docx,pdf'],
        ['整个系统总代码量', '待统计', '使用 cloc . --exclude-dir=node_modules,target,.m2repo,.venv,__pycache__,dist,.git'],
    ]
)
